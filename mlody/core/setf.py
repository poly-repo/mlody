...
from mlody.core.virtual_value import is_virtual_value, iter_virtual_children, step_virtual_value
...
def _step(current: object, segment: object) -> object:
    ...
    if isinstance(segment, IndexSegment):
        if not isinstance(current, (list, tuple)):
            raise TypeError(f"IndexSegment requires a sequence, got {type(current).__name__}")
    ...
    if isinstance(segment, SliceSegment):
        if not isinstance(current, (list, tuple)):
            raise TypeError(f"SliceSegment requires a sequence, got {type(current).__name__}")
        return list(current[slice(segment.start, segment.stop, segment.step)])
    raise NotImplementedError(f"selector segment {type(segment).__name__} is not supported yet")
...
def _children(current: object) -> list[tuple[object, object]]:
    ...
    if is_virtual_value(current):
        assert isinstance(current, Struct)
        return [(FieldSegment(name), value) for name, value in iter_virtual_children(current)]
    if isinstance(current, Struct):
        return [(FieldSegment(name), value) for name, value in current.as_mapping().items()]
...
def _resolve_contract(values: list[object]) -> tuple[object | None, object | None]:
    ...
    for value in values[1:]:
        if getattr(value, "type", None) != declared_type:
            raise ValueError("projected selection does not share a uniform declared type")
        if getattr(value, "representation", None) != representation:
            raise ValueError("projected selection does not share a uniform representation")
...
def _descendants(current: object, prefix: tuple[object, ...]) -> list[tuple[tuple[object, ...], object, object]]:
...
def _resolve_places_recursive(
    *,
    ...
    if not remaining:
        if owner is None:
            raise ValueError("selector resolved to the root object, not a writable place")
        segment = prefix[-1]
        return [
            _make_direct_place(
                root=root,
    ...
    if isinstance(segment, SliceSegment):
        if tail:
            raise NotImplementedError("slice traversal followed by more segments is not supported yet")
        ...
        return [
            _make_place(
                ...
                lineage_selector=PathExpression(segments=prefix[:-1]) if _has_lineage(owner) else None,
            )
        ]
    ...
    if isinstance(segment, RecursiveDescentSegment):
        places: list[Place] = []
        for descendant_prefix, descendant_owner, descendant in _descendants(current, prefix):
            try:
                places.extend(
                    _resolve_places_recursive(
                        root=root,
            ...
            except (AttributeError, IndexError, KeyError, NotImplementedError, TypeError):
                continue
        return places
...
def _replace_one(container: object, segment: object, new_child: object) -> object:
    ...
    if isinstance(segment, FieldSegment):
        if not isinstance(container, Struct):
            raise TypeError(f"FieldSegment requires a Struct, got {type(container).__name__}")
        updated = dict(container.as_mapping())
        updated[segment.name] = new_child
        return Struct(**updated)
    if isinstance(segment, IndexSegment):
        if not isinstance(container, list):
            raise TypeError(f"IndexSegment requires a list, got {type(container).__name__}")
        updated = list(container)
        updated[segment.index] = new_child
        return updated
    if isinstance(segment, KeySegment):
        if not isinstance(container, dict):
            raise TypeError(f"KeySegment requires a dict, got {type(container).__name__}")
...
def _rebuild_owner(root: object, prefix: tuple[object, ...], new_owner: object) -> object:
...
def _replace_path_value(root: object, path: tuple[object, ...], new_value: object) -> object:
    ...
    *,
    ...
    for place in place_set.places:
        ...
        if len(fresh_place_set.places) != 1:
            raise ValueError("concrete place selector did not resolve to exactly one place")
        ...
        if fresh_place.lineage_selector is not None:
            ...
            if not fresh_place.lineage_selector.segments:
                ...
                sink = resolve_places(working_root, fresh_place.lineage_selector).places[0].current_value
